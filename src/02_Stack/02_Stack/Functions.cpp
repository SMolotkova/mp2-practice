...
string  myCalculator::PostfixForm(string your_expression)
{
	...
	int expression_size = your_expression.length();//ÔÓ·Ó‚‡Ú¸ ‚ ËÌÚ
	...
	for (int i = 0; i < expression_size; i++)
	{
		...
		//if a letter came
		if (isalpha(your_expression[i]))//ÂÒÎË ÚÂÍÛ˘ËÈ ÒËÏ‚ÓÎ ÒÚÓÍË ·ÛÍ‚‡
			Operands.Push(your_expression[i]);


		if (your_expression[i] == ')')//Õ¿ƒŒ À» Ã≈Õﬂ“‹ »À» “¿  –¿¡Œ“¿≈“
			while (Signs.top != 0)
			{
	...
	//at the end
	while (!Signs.IsEmpty())
		Operands.Push(Signs.Pop());//Õ”∆ÕŒ À» “”“ ƒŒœ»—¿“‹ 
	string postfix_form;
	while (!Operands.IsEmpty())
	{
		postfix_form += Operands.Pop(); //» “”“
	}
